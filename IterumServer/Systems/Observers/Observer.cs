    ...
    public partial class Observer
    {
        ...
        public void ChangePosition(IThing self, Vector3 pos, bool force = false)
        {
            ...
            if (length > MaxViewDistance/2 || force)
            {
                Debug.Log($"[Observer {ConnectionId}] RequestOverlap Pos: {pos}|{length} LastPos: {LastOverlapPosition}", ConsoleColor.Magenta);
                {
                    ...
                    watch.Stop();
                    Debug.Log($"[Task.Run(() => MakeOverlap())]={watch.ElapsedMilliseconds}ms", ConsoleColor.Gray);
                }
        ...
        private async void MakeOverlap()
        {
            ...
            if (cacheView != null)
            {
                ...
                Debug.Log($"[Observer {ConnectionId}] Overlap Time:{World.Timestamp} Pos:{Position} cache", ConsoleColor.DarkGreen);
            }
            else
            {
                var watch = Perf.StartNew();
                ...
                watch.Stop();
                Debug.Log($"[await World.Overlap(Position, viewList, false)]={watch.ElapsedMilliseconds}ms", ConsoleColor.Gray);
                Debug.Log($"[Observer {ConnectionId}] Overlap Time:{World.Timestamp} Pos:{Position} real", ConsoleColor.DarkMagenta);
                toAdd = list.Item1;
                toRemove = list.Item2;
            }
