...
namespace Magistr.Game
{
    public static class PlayerManager
    {
        ...
        {
            var player = Players[con];
            player.PlayerThing.Boost = data.boost;
        ...
        private static void ChannelPackets_ParseLogin(uint con, Login data)
        {
            ...
            var player = new Player() { ConnectionId = con, PlayerName = "Player #" + Players.Count, ThingTypeId = ThingTypeManager.All.Where(e => e.Category == ThingCategory.Creature).First().ThingTypeId };
